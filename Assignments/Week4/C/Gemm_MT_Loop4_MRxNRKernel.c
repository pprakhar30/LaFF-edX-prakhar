...
void LoopFour( int m, int n, int k, double *A, int ldA, double *B, int ldB,
	       double *C, int ldC )
{
  double *Btilde = ( double * ) _mm_malloc( KC * NC * omp_get_max_threads() * sizeof( double ), 64 );
  double *C_copies = ( double * ) _mm_malloc( m * n * omp_get_max_threads() * sizeof( double ), 64 );
  memset(C_copies, 0, m * n * omp_get_max_threads() * sizeof( double ));

  #pragma omp parallel for
  for ( int p=0; p<k; p+=KC ) {
    int pb = min( KC, k-p );    /* Last loop may not involve a full block */
    PackPanelB_KCxNC( pb, n, &beta( p, 0 ), ldB, &Btilde[KC * NC * omp_get_thread_num()] );
    LoopThree( m, n, pb, &alpha( 0, p ), ldA, &Btilde[KC * NC * omp_get_thread_num()], &C_copies[m * n * omp_get_thread_num()], m );
  }
  ...
  for (int j=0; j<n; j++)
  {
   #pragma omp parallel for
    for(int i=0;i<m; i++)
    {
        ...
        {
            gamma( i,j ) += C_copies[(m*n*t) + (j*m) + (i) ];
        }
    }
  }
